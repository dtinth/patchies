...
const clients: Record<string, ReturnType<typeof makeSocket>> = {};
const topicToInfoHash: Record<string, string> = {};
const infoHashToTopic: Record<string, string> = {};
const announceIntervals: Record<string, Record<string, ReturnType<typeof setInterval>>> = {};
const announceFns: Record<string, Record<string, () => void>> = {};
const trackerAnnounceMs: Record<string, number> = {};
const handledOffers: Record<string, boolean> = {};
const msgHandlers: Record<string, Record<string, (data: any) => void>> = {};
...
const getInfoHash = async (topic: string) => {
...
const send = async (
	client: ReturnType<typeof makeSocket>,
	topic: string,
...
const warn = (url: string, msg: string, didFail?: boolean) =>
	console.warn(
		`${libName}: torrent tracker ${didFail ? 'failure' : 'warning'} from ${url} - ${msg}`
	);

export const joinRoom = strategy({
	init: (config: { relayUrls?: string[]; relayRedundancy?: number; appId: string }) =>
		getRelays(config, defaultRelayUrls, defaultRedundancy).map((rawUrl) => {
			const client = makeSocket(rawUrl, (rawData) => {
				...
				if (interval && interval * 1000 > trackerAnnounceMs[rawUrl] && announceFns[rawUrl][topic]) {
					const int = Math.min(interval * 1000, maxAnnounceMs);

					clearInterval(announceIntervals[rawUrl][topic]);
					trackerAnnounceMs[rawUrl] = int;
					announceIntervals[rawUrl][topic] = setInterval(announceFns[rawUrl][topic], int);
				}
	...
	subscribe: (client, rootTopic, _selfTopic, onMessage, getOffers) => {
		const { url } = client;

		const announce = async () => {
			const offers = fromEntries(
				(await getOffers(offerPoolSize)).map((peerAndOffer) => [genId(hashLimit), peerAndOffer])
			);
...
export { selfId, pauseRelayReconnection, resumeRelayReconnection } from './utils';
